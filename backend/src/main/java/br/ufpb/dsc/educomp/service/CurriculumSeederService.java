...
import br.ufpb.dsc.educomp.config.CurriculumSeeder.SeedSlide;
...
public class CurriculumSeederService {
    ...
    public CurriculumSeederService(AulaRepository aulaRepository) {
        this.aulaRepository = aulaRepository;
    }

    @Transactional
    public void povoarESalvarAulaTransacional(Long aulaId, List<SeedSlide> customSlides) {
        Aula aula = aulaRepository.findById(aulaId)
                .orElseThrow(() -> new IllegalArgumentException("Aula não encontrada: " + aulaId));

        List<Slide> slides = new ArrayList<>();

        if (customSlides != null && !customSlides.isEmpty()) {
            for (SeedSlide seed : customSlides) {
                slides.add(Slide.builder()
                        ...
                        .tempoEstimado(seed.tempoEstimado != null ? seed.tempoEstimado : 5)
                        .sugestaoExplicacao(seed.sugestaoExplicacao)
                        .perguntasSugeridas(seed.perguntasSugeridas)
                        .curiosidades(seed.curiosidades)
                        .errosComuns(seed.errosComuns)
                        .materiaisComplementares(seed.materiaisComplementares)
                        .build());
            }
        } else {
            // Fallback Gerador de Conteúdo contextualizado de alta qualidade
            slides.add(Slide.builder()
                    .aula(aula)
        ...
        // Atualizar slides com segurança
        ...
        // Criar ou atualizar Quiz de forma segura
        ...
        if (quiz.getPerguntas() == null) {
            ...
            quiz.getPerguntas().addAll(perguntas);
        }
